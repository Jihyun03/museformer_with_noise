...
class RpeSelfAttentionV2S1(nn.Module):
    ...
    def forward(
        ...
        *args, **kwargs,
        ...
        # ===== CS470 - apply adaptive noise to inputs =====
        #apply_noise = True # Whether to apply noise or not
        #noise_level = torch.sum(torch.exp(torch.tensor([-1*self.layer_idx]).cuda()))
        #noise_level = math.exp(-1 * self.layer_idx)
        #noise_level = torch.tensor(noise_level).cuda()
        #if apply_noise:
        #    if sum_x is not None:
        #        sum_x += (noise_level 
        #                  * torch.linalg.norm(sum_x)
        #                  / math.sqrt(torch.numel(sum_x))
        #                  * torch.randn(sum_x.size()))
        #    if reg_x is not None:
        #        reg_x += (noise_level 
        #                  * torch.linalg.norm(reg_x)
        #                  / math.sqrt(torch.numel(reg_x))
        #                  * torch.randn(reg_x.size()))

        # ==================================================
        apply_noise = True # Whether to apply noise or not
        noise_level = math.exp(-2 * self.layer_idx)
        if apply_noise:
            if sum_x is not None:
                noise = torch.randn(sum_x.size(), dtype=sum_x.dtype).cuda()
                if math.sqrt(torch.numel(sum_x)) != 0:
                    constant =(noise_level 
                            * torch.linalg.norm(sum_x).tolist()
                            / math.sqrt(torch.numel(sum_x)))
                else:
                    constant = 0
                noise = torch.mul(noise, constant)
                sum_x = torch.add(sum_x, noise)
            if reg_x is not None:
                noise = torch.randn(reg_x.size(), dtype=reg_x.dtype).cuda()
                if math.sqrt(torch.numel(reg_x)) != 0:
                    constant =(noise_level 
                            * torch.linalg.norm(reg_x).tolist()
                            / math.sqrt(torch.numel(reg_x)))
                else:
                    constant = 0
                noise = torch.mul(noise, constant)
                reg_x = torch.add(reg_x, noise)
